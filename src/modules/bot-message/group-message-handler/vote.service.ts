...
export class VoteKickService {
    ...
    private inVoteKickIdTimerMap: object = {} // 储存当前被投用户对应的计时器
    private currentVotedMemberMap: object = {} // 储存当前被投用户对应的已投票人员
    private memberVotesMap: object = {} // 当前被投用户票数
    ...
    // 投票踢人逻辑
    async handleVoteKick(roomId: number, voteKickId: number, otherAtMember: Array<number>) {
        const targetMember = otherAtMember[0];
        const isVoted = this.checkUserIsVoted(targetMember, voteKickId);
        const currentMemberVotes = await this.getMemberVotes(targetMember, voteKickId)
        ...
        if (isVoted) {
            ...
            if (!this.inVoteKickProgressList.includes(targetMember)) {
                this.inVoteKickIdTimerMap[targetMember] = this.setVoteKickTime(this.inVoteKickProgressList.push(targetMember) - 1);
                statusText = '开启'
            }
            messageChain = [{
                type: MessageChainItemType.Plain,
            ...
            // 清理流程中账号与计时器和计数
            this.inVoteKickProgressList.splice(this.inVoteKickProgressList.findIndex(item => item === targetMember), 1)
    ...
    async getUserName(userId, roomId): Promise<string> {
        const targetMemberInfo = await this.commonService.getUserInfo(userId, roomId);
        const username = targetMemberInfo?.specialTitle || targetMemberInfo?.name || '未知用户';
        return username
    }
    ...
    setVoteKickTime(index: number): NodeJS.Timeout {
        return setTimeout(() => {
            // 倒计时结束如果还未清除的话
            this.inVoteKickProgressList[index] && this.inVoteKickProgressList.splice(index, 1);
        }, 5 * 60 * 1000)
    }
    ...
    async handlerRemoveProgress(memberId, roomId): Promise<boolean> {
        ...
        const result = await this.userService.kickMember(memberId, roomId, '您被投票移出群聊');
        return result;
    }

    // 计票
    async getMemberVotes(memberId: number, voteKickId: number): Promise<number> {
        let current = this.memberVotesMap[memberId] || 0;
        if (this.currentVotedMemberMap[memberId]?.includes(voteKickId)) return current;
        current++;
