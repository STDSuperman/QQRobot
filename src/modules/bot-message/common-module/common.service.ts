...
import { MemberInfo } from '@modules/user/user.interface';
...
import { SendFriendMessage } from '@modules/bot-send/interface/send.interface';
...
export class CommonService {
	...
	checkAtRobotAndInclueKey(
		...
		const otherAtMember: Array<number> = [];
		message.messageChain.find((item) => {
			...
			if (
				item.type === MessageChainItemType.Plain &&
	...
	async sendMessageToGroupList(
		...
		const targetGroupList = isIncludeCurrent
			...
			: this.getOtherConnectionGroupList(message?.sender?.group?.id);
	...
	getOtherConnectionGroupList(id: number): Array<number> {
		// 如果存在缓存则无需重新计算
		if (this.cacheOtherConnectionGroup[id])
			return this.cacheOtherConnectionGroup[id];
		const result = this.connectionGroupList.reduce(
