...
export class TianXingMessageService {
    ...
    async handleIsGetTianGou(message: MessageType): Promise<void> {
        const { checkRes } = this.commonService.checkAtRobotAndInclueKey(message, '舔狗日记', this.isFriendMessage);
        if (checkRes) {
            ...
            this.isFriendMessage ? 
                this.commonService.sendFriendMessage(message as FriendMessage, tianGouOneContent) :
                this.commonService.sendMessageToGroupList(message as GroupChatMessage, true);
        }
    }

    // 判读是否获取彩虹屁
    async handlerIsGetRainbowFart(message: MessageType): Promise<void>{
        const { checkRes } = this.commonService.checkAtRobotAndInclueKey(message, '彩虹屁', this.isFriendMessage);
        if (checkRes) {
            ...
            this.isFriendMessage ? 
                this.commonService.sendFriendMessage(message as FriendMessage, tianRainbowFart) :
                this.commonService.sendMessageToGroupList(message as GroupChatMessage, true);
        }
    }
    ...
    async handlerSendMessage(message: MessageType, text: string) {
        ...
        this.isFriendMessage ? 
            this.commonService.sendFriendMessage(message as FriendMessage, text) :
            this.commonService.sendMessageToGroupList(message as GroupChatMessage, true);
    }
}